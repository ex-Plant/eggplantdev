...
const SECTION_REGISTRY: { id: string; label: string; component: (colorful: boolean) => React.ReactNode }[] = [
  ...
  {
    id: "ecliptic-availability",
    label: "Ecliptic Availability",
    component: (c) => <EclipticAvailability colorful={c} />,
  },
  ...
  notes: [
    {
      eyebrow: "01",
      title: "Speed Is Felt",
      text: "Interfaces are judged before they are understood. Perceived responsiveness is part of the design, not a technical afterthought.",
      size: "tall",
    },
    {
      eyebrow: "02",
      title: "Clarity Wins",
      text: "If a feature needs a paragraph to justify itself, the UI probably still needs another pass.",
      size: "standard",
    },
    {
      eyebrow: "03",
      title: "Real Users Break Nice Systems",
      text: "The structure only proves itself when content is messy, priorities shift, and people use the product in ways nobody planned.",
      size: "wide",
    },
    {
      eyebrow: "04",
      title: "Leave Room",
      text: "Good systems are specific enough to guide the work and loose enough to survive iteration.",
      size: "standard",
    },
    {
      eyebrow: "05",
      title: "Polish Matters",
      text: "Tiny choices around spacing, motion, and copy tone often decide whether a product feels considered or generic.",
      size: "standard",
    },
  ],
  buttons: [
...
function FieldNotesSection({ data, className }: FieldNotesSectionPropsT) {
  switch (data.variant) {
    ...
    case "neonLounge":
      return <NeonLoungeNotes data={data} className={cn("", className)} />;
    case "topoContour":
      return <TopoContourNotes data={data} className={cn("", className)} />;
    case "sacredNeon":
      return <SacredNeonNotes data={data} className={cn("", className)} />;
    case "hexGrid":
      return <HexGridNotes data={data} className={cn("", className)} />;
    case "circuitTrace":
      return <CircuitTraceNotes data={data} className={cn("", className)} />;
    default:
      return null;
  }
}
...
const FIELD_NOTES_ITEMS = VARIANT_REGISTRY.map(({ key, label }) => ({
  ...
  label,
  component: (
    <FieldNotesSection data={{ ...BASE_DATA, variant: key, id: `field-notes-${key}` }} className="py-12 md:py-20" />
  ),
}));
  ...
  return (
    <div className="bg-bgc min-h-screen text-white">
      ...
      {/* ══════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════ */}
      <div className="fest-container mt-12 border-t border-green-500/20 pt-12">
        <h2 className="text-24 md:text-40 mb-2 font-mono text-green-500/60 uppercase">Field Notes Variants</h2>
