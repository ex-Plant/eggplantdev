...
import { PALETTE, PALETTE_GOLD, FLOWER_CIRCLES, GEOMETRY, COPY, buildNebulaClouds } from "./config";
  ...
  return (
    <div id="hero-cosmic-aubergine" className="relative flex min-h-screen items-center overflow-hidden">
      ...
      <div className="fest-container relative z-10 grid gap-16 md:grid-cols-2 md:items-center">
        {/* Left: text */}
        <div>
          <p className="text-16 font-mono tracking-[0.3em] uppercase" style={{ color: `${p.accent1}80` }}>
            {COPY.subtitle}
          </p>
          <h1 className="text-64 mt-6 font-mono leading-none text-white uppercase md:text-[5.5rem]">
            {COPY.titleLine1}
            <br />
            <span style={{ color: p.accent2 }}>{COPY.titleLine2}</span>
            <br />
            {COPY.titleLine3}
          </h1>
          <p className="text-20 mt-8 max-w-md leading-relaxed text-white/35">{COPY.description}</p>
          <div className="mt-10 flex gap-4">
            <span
              className="rounded-full border px-5 py-2.5 font-mono text-sm tracking-wider uppercase"
              style={{ borderColor: `${p.accent1}4d`, color: `${p.accent1}99` }}
            >
              {COPY.buttons[0]}
            </span>
            <span
              className="rounded-full border px-5 py-2.5 font-mono text-sm tracking-wider uppercase"
              style={{ borderColor: `${p.accent2}4d`, color: `${p.accent2}99` }}
            >
              {COPY.buttons[1]}
            </span>
          </div>
