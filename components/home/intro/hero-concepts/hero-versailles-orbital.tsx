/* Agent: Claude — Versailles Orbital */

const ORBITS = [
  { rx: 120, ry: 100, stroke: "#ffd700", bodies: [0, 2.2, 4.5] },
  { rx: 190, ry: 155, stroke: "#daa520", bodies: [1.1, 3.8] },
  { rx: 270, ry: 210, stroke: "#c8860e", bodies: [0.7, 2.9, 5.2] },
  { rx: 350, ry: 280, stroke: "#daa520", bodies: [1.8, 4.1] },
];

export function HeroVersaillesOrbital() {
  return (
    <div id="hero-versailles-orbital" className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0a0806]">
      {/* Star field */}
      {Array.from({ length: 55 }, (_, i) => (
        <div key={`s-${i}`} className="absolute rounded-full" style={{
          left: `${(i * 19 + 7) % 100}%`, top: `${(i * 37 + 13) % 100}%`,
          width: i % 7 === 0 ? 2.5 : 1, height: i % 7 === 0 ? 2.5 : 1,
          background: i % 3 === 0 ? "#ffd700" : i % 3 === 1 ? "#daa520" : "#f5e6c0",
          opacity: 0.04 + (i % 5) * 0.018,
        }} />
      ))}

      {/* Sacred geometry SVG */}
      <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 1200 800" preserveAspectRatio="xMidYMid slice">
        {/* Art deco double-line corner frames */}
        {[[40, 40, 0], [1120, 40, 90], [1120, 720, 180], [40, 720, 270]].map(([x, y, r]) => (
          <g key={`c-${r}`} transform={`translate(${x},${y}) rotate(${r})`}>
            <path d="M 0 40 L 0 0 L 40 0" fill="none" stroke="#ffd700" strokeWidth="0.8" opacity="0.12" />
            <path d="M 0 50 L 0 -10 L 50 -10" fill="none" stroke="#daa520" strokeWidth="0.4" opacity="0.08" />
          </g>
        ))}

        {/* Bilateral symmetry axes */}
        <line x1="600" y1="100" x2="600" y2="700" stroke="#c8860e" strokeWidth="0.4" opacity="0.1" />
        <line x1="200" y1="400" x2="1000" y2="400" stroke="#c8860e" strokeWidth="0.4" opacity="0.1" />

        {/* Tick marks along vertical axis */}
        {Array.from({ length: 25 }, (_, i) => {
          const y = 150 + i * 24;
          const w = i % 5 === 0 ? 8 : 3;
          return <line key={`tk-${i}`} x1={600 - w} y1={y} x2={600 + w} y2={y} stroke="#daa520" strokeWidth={i % 5 === 0 ? 0.7 : 0.3} opacity="0.1" />;
        })}

        {/* Decorative scroll curves at axis intersection (Versailles ironwork) */}
        {[1, -1].map((sx) => [1, -1].map((sy) => (
          <path key={`sc-${sx}-${sy}`} d={`M ${600 + sx * 8} 400 Q ${600 + sx * 20} ${400 + sy * 20} ${600} ${400 + sy * 8}`} fill="none" stroke="#ffd700" strokeWidth="0.6" opacity="0.15" />
        )))}

        {/* Orbital ellipses with planetary bodies */}
        {ORBITS.map(({ rx, ry, stroke, bodies }, i) => (
          <g key={`orb-${i}`}>
            <ellipse cx="600" cy="400" rx={rx} ry={ry} fill="none" stroke={stroke} strokeWidth={0.5} opacity={0.12 - i * 0.02} />
            {bodies.map((a, j) => {
              const rad = a;
              return <circle key={`p-${i}-${j}`} cx={600 + rx * Math.cos(rad)} cy={400 + ry * Math.sin(rad)} r={j === 0 ? 3 : 2} fill={stroke} opacity={0.15} />;
            })}
          </g>
        ))}
      </svg>

      {/* Central content */}
      <div className="relative z-10 flex flex-col items-center text-center">
        {/* Golden radial glow */}
        <div className="absolute top-1/2 left-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#daa520]/5 blur-3xl" />
        <div className="absolute top-1/2 left-1/2 h-44 w-44 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#ffd700]/8 blur-2xl" />

        {/* Central eggplant */}
        <img src="/logos/eggplant-logo-smooth.apng" alt="" className="relative h-48 w-48 object-contain" style={{ filter: "sepia(0.3) saturate(1.6) brightness(1.1)" }} />

        {/* Eyebrow */}
        <div className="mt-6 flex items-center gap-3">
          <div className="h-px w-12 bg-[#ffd700]/15" />
          <span className="font-mono text-[10px] uppercase tracking-[0.5em] text-[#ffd700]/35">Mécanique céleste</span>
          <div className="h-px w-12 bg-[#ffd700]/15" />
        </div>

        {/* Title */}
        <h1 className="mt-4 font-mono text-48 uppercase text-[#f5e6c0] md:text-72">
          Versailles<br /><span className="text-[#ffd700]">Orbital</span>
        </h1>

        {/* Subtitle */}
        <p className="mt-6 max-w-md font-mono text-14 text-[#c8b080]/45">
          The eggplant holds court at the center of a gilded orrery — all cosmic bodies in faithful orbit around its aubergine mass, tracing gold ellipses through the hall of mirrors.
        </p>
      </div>
    </div>
  );
}
