...
export function HeroMetatronsCubePlayground() {
  ...
  return (
    <div
      id="hero-metatrons-cube"
      className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0c0a08]"
    >
      {/* Star field */}
      {STARS.map((s, i) => (
        <div
      ...
      >
        ...
        {ALL_POINTS.map(([cx, cy], i) => (
          <circle
            ...
            cx={cx}
            cy={cy}
            r={i === 0 ? INNER_R : i <= 6 ? INNER_R * 0.65 : INNER_R * 0.65}
            fill="none"
            stroke={STROKES[i % 3]}
        ...
        <rect
          ...
          strokeWidth="0.4"
          opacity="0.08"
          transform="rotate(45 750 395)"
        />
      </svg>

      {/* Radial glow */}
      <div className="pointer-events-none absolute top-1/2 left-1/2 h-[480px] w-[480px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(circle,rgba(218,165,32,0.12)_0%,rgba(200,134,14,0.05)_40%,transparent_70%)]" />

      {/* Content */}
      <div className="relative z-10 flex flex-col items-center text-center">
        <p className="mb-6 font-mono text-xs tracking-widest text-[#daa520]/40 uppercase">
        ...
        <p className="mt-6 max-w-md font-mono text-sm text-[#c8b080]/50">
          The eggplant exists within the geometric template of the universe — encoded in every vertex, every edge, every
          platonic form
        </p>
      </div>
    </div>
